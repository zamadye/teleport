...
received a TCP packet with a control bit set to RST (reset). A TCP server sends
an RST packet if it receives a packet that is unexpected given its connection
state, e.g., if the other side unexpectedly exits mid-connection.
...
proxying a connection exited without closing the connection. A device like a
firewall or load balancer may also be to blame.
...
As explained in
[Troubleshooting](../../zero-trust-access/management/diagnostics/troubleshooting.mdx#teleport-cluster-local),
...
use [TLS
multiplexing](../../zero-trust-access/deploy-a-cluster/tls-routing.mdx), but you
