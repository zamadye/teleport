...
<Admonition type="note">
  Use `tsh` client version 18.8.x or later in order to use the `tsh aws-profile` command.
</Admonition>

The `tsh aws-profile` command automatically generates native AWS SSO profiles in `~/.aws/config` from your AWS Identity Center integration data.
...
3. Authenticate with AWS SSO:
   ...
   This will open a browser window to complete the authentication process via Teleport.
...
5. Switch to another profile:
   ...
   You can quickly switch to another profile at any time by exporting a different `AWS_PROFILE` value.
